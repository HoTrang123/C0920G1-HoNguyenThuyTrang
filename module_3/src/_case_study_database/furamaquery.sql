...
select *
 from nhanvien
 where ((nhanvien.ho_ten like 'K%') 
or (nhanvien.ho_ten like 'H%') or (nhanvien.ho_ten like 'T%')) 
 and length(furamaresort.nhanvien.ho_ten) <= 15;

-- Hiển thị thông tin của tất cả khách hàng có độ tuổi từ 18 đến 50 tuổi 
-- và có địa chỉ ở “Đà Nẵng” hoặc “Quảng Trị”.

select * from furamaresort.khachhang 
where (date_sub(now() , interval 18 year) <= khachhang.ngay_sinh 
and date_sub(now(),interval 50 year) >= khachhang.ngay_sinh) 
and (khachhang.diachi like "Da Nang" or khachhang.diachi like "Quang Tri");

-- đếm xem mỗi khách hàng đặt phòng bao nhiêu lần. hiển thị sắp xếp tăng dần theo số phòng. 
-- và khách hàng là diamond
